...
void DS3231_display()
{
 ...
 times[1]     = hour   % 10 + 48;
 times[0]     = hour   / 10 + 48;
 Calender[9]  = year   % 10 + 48;
 Calender[8]  = year   / 10 + 48;
 Calender[4]  = month  % 10 + 48;
 Calender[3]  = month  / 10 + 48;
 Calender[1]  = date   % 10 + 48;
 Calender[0]  = date   / 10 + 48; // Menampilkan waktu
 lcd.setCursor(10, 0);
 lcd.print(Calender); // Menampilkan kalender
 lcd.setCursor(0, 0);
 lcd.print(times);
 lcd.setCursor(1, 1);
}
...
void sendUpdate()
{
 ...
 if(sd_ok)  
 {                            
  // Jika inisialisasi SDcard OK
  dataLog = SD.open("Logger.txt", FILE_WRITE); // // Buka file Logger.txt
  if(dataLog)
  {                                   
   // jika file terbuka dengan baik, tulislah:
   dataLog.print(Calender);
   dataLog.print(" | ");
   dataLog.print(times);
   dataLog.print(" |   ");
